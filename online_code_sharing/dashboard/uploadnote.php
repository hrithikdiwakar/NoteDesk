...
if (isset($_SESSION['id'])) {
    ...
    $validExt = array ('pdf', 'txt', 'doc', 'docx', 'ppt' , 'zip','py','c','HTML','java','php','css','js');
    if (empty($file)) {
...
?>
	...
	<div class="form-group">
		<label for="post_title">Code File Title</label>
		<input type="text" name="title" class="form-control" placeholder="Eg: Python File"  value = "<?php if(isset($_POST['upload'])) {
            echo $file_title; } ?>" required="">
	</div>

	<div class="form-group">
		<label for="post_tags">Short Code file Description</label>
		<input type="text" name="description" class="form-control" placeholder="Eg: A python script to find fibonnacii sequence..." value="<?php if(isset($_POST['upload'])) {
            echo $file_description;  } ?>" required="" ">
	</div>

	 <div class="form-group">
        <label for="post_image">Select File</label><font color="brown"> (allowed file type: 'c','python','java','html','js' | allowed maximum size: 30 mb ) </font>
		<input type="file" name="file"> 
     </div>

<button type="submit" name="upload" class="btn btn-primary" value="Upload Note">Upload Code File</button>
<br>
<br>
</form>
</div>
